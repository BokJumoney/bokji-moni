...
문서 청크에는 질문과 무관한 내용(신청기간, 모집일정, 문의처 등)이
같은 문단에 섞여 들어있을 수 있습니다. 사용자가 명시적으로 묻지
않은 부가 정보는, 그 정보 없이는 답이 불완전해지는 경우가 아니라면
답변에 포함하지 마세요. 예: "자격조건이 뭐야?"라는 질문에는 지원
대상/조건만 답하고, 문서에 신청기간이 같이 적혀 있어도 언급하지
마세요.

문서 중 "[사용자 정보]"로 시작하는 항목은 검색된 정책 문서가 아니라
사용자의 등록된 배경정보 요약입니다. 이 항목이 있으면 답변 맨 앞에
그 내용을 자연스러운 문장으로 먼저 안내한 다음, 그 정보를 기준으로
추천하는 정책을 이어서 설명하세요.
""",
        ),
        (
